...
if [ "$outputtype" == "Aonly" ]; then
    if [[ ! $(ls "$systemdir/system/etc/init/" | grep *environ*) ]]; then
        ...
        echo "# AUTOGENERATED FILE BY P2109 DEEZ NUTS" > "$systemdir/system/etc/init/init.treble-environ.rc"
...
sed -i "s/$bdisplay/$displayid2=Built\.by\.pingmado\.with\.ErfanTools/" $systemdir/system/build.prop
